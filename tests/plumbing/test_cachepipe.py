...
def test_cachepipe_writer(input, func_add_one, tmp_path):
    ...
    first_cache_file = file.File(
        first_cache_path, intermediate.FORMAT, intermediate.CONTAINER
    )
    ...
    second_cache_file = file.File(
        second_cache_path, intermediate.FORMAT, intermediate.CONTAINER
    )
...
def test_cachepipe_reader(input, func_add_one, tmp_path):
    ...
    first_process.at.parent.thread.process = mock.MagicMock(
        side_effect=first_process.at.parent.thread.process
    )
    second_process.at.parent.thread.process = mock.MagicMock(
        side_effect=second_process.at.parent.thread.process
    )
    result = list(second_process)
    assert len(result) == len(input)
